...
export const ImageLightboxModal = forwardRef<LightboxHandle, Props>(function ImageLightboxModal(
  ...
  const [emblaRef, emblaApi] = useEmblaCarousel({ startIndex: initialIndex, loop: images.length > 1 });
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex flex-col bg-black/94 backdrop-blur-xl animate-fade-in"
      role="dialog"
      aria-modal
    ...
    >
      ...
      <div className="flex items-center justify-between px-8 py-5 shrink-0">
        <div>
          {title && (
            <p className="font-mono text-[10px] uppercase tracking-[0.35em] text-primary">{title}</p>
          )}
